...
readers, announce the dialog. The logic for which element gains focus has
...
by setting the `value` from `false` to a number.
...
example, the dialog widget is now stored in `ui-dailog` instead of `dialog`.
See [the 1.9 deprecation notice](http://jqueryui.com/upgrade-guide/1.9/#changed-naming-convention-for-data-keys) for full details.
